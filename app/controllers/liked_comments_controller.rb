class LikedCommentsController < ApplicationController
  before_action :set_liked_comment, only: %i[ show edit update destroy ]

  # GET /liked_comments or /liked_comments.json
  def index
  ...
  # PATCH/PUT /liked_comments/1 or /liked_comments/1.json
  def update
    respond_to do |format|
      if @liked_comment.update(liked_comment_params)
        format.html { redirect_to liked_comment_url(@liked_comment), notice: "Liked comment was successfully updated." }
